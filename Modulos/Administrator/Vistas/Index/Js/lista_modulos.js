$(document).ready(function () {
    ...
    $('#lista_modulos tbody').on('click', 'tr', function () {
        var id = (jQuery(".selected").attr("id"));
        if (typeof id === 'undefined'){
            $("#eliminar").attr('disabled',true);            
            $("#editarModulo").attr('disabled',true);
        }else{
            $("#eliminar").attr('disabled',false);            
            $("#editarModulo").attr('disabled',false);            
        }
    });
});
...
function eliminar(){    
    ...
    if (typeof name === 'undefined'){
        ...
        $('.modal-body').text('¿Está seguro de querer eliminar el Módulo: ' + name + '?');
...
$(function() {
    $("#btn_dlg_eliminar").click(function() {
        ...
        $.post("?mod=Administrator&cont=index&met=eliminarModulo", {id: seleccionado},
        function(data) {
            if (data > 0){
                alert('Datos eliminados');
                window.location.assign('?mod=Administrator&cont=index&met=modulos');
            }
        });
...
function editar(){    
    ...
    if (typeof seleccionado === 'undefined'){
        ...
        window.location.assign('?mod=Administrator&cont=index&met=editarModulo&id=' + Number(seleccionado));
    }
}
