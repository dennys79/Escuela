$(document).ready(function () {
    ...
    $('#lista_puestos tbody').on('click', 'tr', function () {
        var id = (jQuery(".selected").attr("id"));
        if (typeof id === 'undefined'){
            $("#eliminar").attr('disabled',true);            
            $("#editarPuesto").attr('disabled',true);
        }else{
            $("#eliminar").attr('disabled',false);            
            $("#editarPuesto").attr('disabled',false);            
        }
    });
});
...
function eliminar(){    
    ...
    if (typeof name === 'undefined'){
        ...
        $('.modal-body').text('¿Está seguro de querer eliminar al puesto: ' + name + '?');
...
$(function() {
    $("#btn_dlg_eliminar").click(function() {
        ...
        $.post("?mod=Administrator&cont=index&met=eliminarPuesto", {id: seleccionado},
        function(data) {
            if (data > 0){
                alert('Datos eliminados');
                window.location.assign('?mod=Administrator&cont=index&met=puestos');
            }
        });
...
function editar(){    
    ...
    if (typeof seleccionado === 'undefined'){
        ...
        window.location.assign('?mod=Administrator&cont=index&met=editarPuesto&id=' + Number(seleccionado));
    }
}
