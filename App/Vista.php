...
class App_Vista
{
    ...
    public function __construct(App_Request $peticion, App_Acl $_acl=null)
    {
        ...
        if ($modulo) {
            $this->_rutas['vista'] = MODS_PATH . ucfirst($modulo) . DS . 'Vistas' . DS . ucfirst($controlador) . DS;
//            echo $this->_rutas['vista'];
...
//        echo $rutaVista;
    ...
    public function setCss(array $css)
    {
        if (is_array($css) && count($css)) {
            foreach ($css as $archivoCss) {
                $this->_css[] = BASE_URL . 'Vistas/Layout/' . DEFAULT_LAYOUT . '/Css/' . $archivoCss . '.css';
            }
        } else {
            throw new Exception('Error de Css');
        }
