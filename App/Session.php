...
class App_Session
{
    ...
    public static function acceso($level)
    {
        if (!Session::get('autenticado')) {
            header('location:' . BASE_URL . '?mod=error&cont=acceso&id=5050');
            exit;
        }
    ...
    public static function tiempo()
    {
        ...
        if (time() - App_Session::get('tiempo') > (SESSION_TIME * 60)) {
            App_Session::destroy();
            header('location:' . BASE_URL . '?mod=Error&cont=Error&met=e8080');
            exit;
        } else {
    ...
    {
        echo 'Autenticado: ' . App_Session::get('autenticado') . '<br>';
        echo 'Level: ' . App_Session::get('level') . '<br>';
        echo 'Usuario: ' . App_Session::get('usuario') . '<br>';
    }
    
    public static function isAutenticado()
    {
        if (!App_Session::get('autenticado') || App_Session::tiempo()){
            header('location:' . BASE_URL . '?'. URL_LOGIN);
            exit;
        }
    }
