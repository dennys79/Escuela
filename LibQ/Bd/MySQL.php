...
class MySQLAdapter implements ManejadorBaseDeDatosInterface
{
    ...
    public function query(SQL $sql)
    {
        ...
        if (!$this->_result = mysqli_query($this->_link, $sql)) {
            //Este no porque muesta el nombre de la BD
            //throw new MySQLAdapterException('Error ejecutando la consulta ' . $sql . mysqli_error($this->_link));
            throw new MySQLAdapterException('Error ejecutando la consulta ' . $sql, self::NO_HAY_TABLA);
        }
        return $this->_result;
    }
