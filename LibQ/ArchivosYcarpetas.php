...
class LibQ_ArchivosYcarpetas
{
     ...
     */
    public static function listarArchivos($directorio)
    {
        $ficheros = array();
        if (is_dir($directorio)){
            ...
            for ($index = 0; $index <= count($ficheros); $index++) {
                if (self::isDirectory($ficheros[$index])){
                    unset($ficheros[$index]);
                }
            }
        }
        return $ficheros;
    ...
    public static function isDirectory($directorio)
    {
        return is_dir($directorio);
    }
